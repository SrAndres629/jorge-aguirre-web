...
async def retry_create():
    headers = {"apikey": API_KEY, "Content-Type": "application/json"}
    print(f"🔄 RETRYING CREATION FOR: {INSTANCE}")
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        if resp.status_code == 201 or resp.status_code == 400: # 400 might mean exists
             # 2. GET QR
             ...
             if resp.status_code == 200:
                ...
                if b64:
                    ...
                    <body style="background-color: #1a1a1a; display: flex; justify-content: center; align-items: center; height: 100vh; flex-direction: column; color: white; font-family: sans-serif;">
                        <h1>🤖 Natalia Brain Hard Reset (Attempt 2)</h1>
                    ...
                    """
                    with open("qr_retry.html", "w") as f:
                        f.write(html_content)
                    print("\n✅ QR CODE GENERATED: qr_retry.html")
                    abs_path = os.path.abspath("qr_retry.html")
                    webbrowser.open(f"file://{abs_path}")
             else:
                 print(f"❌ Connect Failed: {resp.status_code} | {resp.text}")

if __name__ == "__main__":
    asyncio.run(retry_create())
